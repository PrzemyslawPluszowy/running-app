...
class _ListCalutedCreanState extends State<ListCalutedCrean> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ListRaceCalculatedCubit, ListRaceCalculatedState>(
      ...
      if (state is ListRaceCalculatedLoading ||
          state is ListRaceCalculatedInitial) {
        return const Center(child: CircularProgressIndicator());
      }
      if (state is ListRaceCalculatedLoaded) {
        if (state.listUserRace.isEmpty) {
          return const Center(
              child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text("Oh no! People don't use this app :("),
              Icon(Icons.nature_people, size: 100),
            ],
          ));
        } else {
